...
class Table extends React.Component{
    ...
    componentDidMount(){
        ...
        for(let key in bodyOperate){
            $('#' + this.props.id + '_table').delegate('.'+ id + '-service-provider'+key, 'click', bodyOperate[key]);
        }
    }
    ...
    createBody(){
        ...
        for(var i=0; i<len; i++){
            ...
            for(var j=0; j<objLen; j++){
                ...
                statusOperateTemp === undefined ? finalTag = <a href="javascript:;" className={id + '-service-provider'+j+' '+style} data-index={i}>{content}</a>:
                                                  finalTag = <BtnGroup style={style} btnNames={statusOperateTemp} identification={id + '-service-provider'+j} index={i}/>;
                if(operate[j] !== undefined)   finalTag = <BtnGroup style={style} btnNames={operate[j]} identification={id+'-service-provider'+j} index={i}/>;
