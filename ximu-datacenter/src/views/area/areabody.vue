<template>
	<div>
		<!-- 头部筛选项 -->
					...
					<div>
						<Row>
					        <Col span="8"><div id="pie1" style="height: 200px"></div><p>合计：{{pie1total}}</p><p>白名单对比</p></Col>
					        <Col span="8"><div id="pie2" style="height: 200px"></div><p>合计：{{pie2total}}</p><p>已开通对比</p></Col>
					        <Col span="8"><div id="pie3" style="height: 200px"></div><p>合计：{{pie3total}}</p><p>已支付对比</p></Col>
					    </Row>
					    <Row>
					...
					<div>
						...
						<div>
						<Row>
					        <Col span="8"><div id="citypie1" style="height: 200px"></div><p>合计：{{citypie1total}}</p><p>白名单对比</p></Col>
					        <Col span="8"><div id="citypie2" style="height: 200px"></div><p>合计：{{citypie2total}}</p><p>已开通对比</p></Col>
					        <Col span="8"><div id="citypie3" style="height: 200px"></div><p>合计：{{citypie3total}}</p><p>已支付对比</p></Col>
					    </Row>
					...
					<div>
						...
						<Row>
					        <Col span="8"><div id="dispie1" style="height: 200px"></div><p>合计：{{dispie1total}}</p><p>白名单对比</p></Col>
					        <Col span="8"><div id="dispie2" style="height: 200px"></div><p>合计：{{dispie2total}}</p><p>已开通对比</p></Col>
					        <Col span="8"><div id="dispie3" style="height: 200px"></div><p>合计：{{dispie3total}}</p><p>已支付对比</p></Col>
					    </Row>
						<div>
                        	...
                        	return h('div', [
                        		h('p', {
                        			style: {
                        				height: '25px',
                        				background: '#61A0A8',
                        				width: params.row.paid/this.city_rate3*100+ '%'
                        			}
                        		})
                        		])
                        	}
                        	...
                        	if (this.dis_rate3 == 0) {
                        		...
                        		h('p', {
                        			style: {
                        				height: '25px',
                        				background: '#61A0A8',
                        				width: params.row.paid/this.dis_rate3*100+ '%'
                        			}
                        		})
                        		])
                        	}
                        	...
                        	if (this.pro_rate3 == 0) {
                        		...
                        		h('p', {
                        			style: {
                        				height: '25px',
                        				background: '#61A0A8',
                        				width: params.row.paid/this.pro_rate3 *100+ '%'
                        			}
                        		})
                        		])
                        	} 
		...
		methods: {
			...
			pro_pie1 () {
				...
				for (var i in _this.pro_data) {
						var item = {name: i, value: _this.pro_data[i].white_list};
						pie1data.push(item);
						_this.pie1total +=  _this.pro_data[i].white_list;
					};
				pie1.setOption({
					tooltip: {
				        trigger: 'item',
			...
			pro_pie2 () {
				var pie2 = eCharts.init(document.getElementById('pie2')),
					...
					pie2data = [];
				for (var i in _this.pro_data) {
						var item = {name: i, value: _this.pro_data[i].opened};
						_this.pie2total += _this.pro_data[i].opened;
						pie2data.push(item);
					};
			...
			pro_pie3 () {
				var pie3 = eCharts.init(document.getElementById('pie3')),
					...
					pie3data = [];
				for (var i in _this.pro_data) {
						var item = {name: i, value: _this.pro_data[i].paid};
						_this.pie3total += _this.pro_data[i].paid;
						pie3data.push(item);
					};
			...
			city_pie1 () {
				...
				for (var i in _this.city_data) {
						var item = {name: i, value: _this.city_data[i].white_list};
						_this.citypie1total += _this.city_data[i].white_list;
			...
			city_pie2 () {
				var pie3 = eCharts.init(document.getElementById('citypie3')),
					...
					citypie3data = [];
				for (var i in _this.city_data) {
						var item = {name: i, value: _this.city_data[i].paid};
						_this.citypie3total += _this.city_data[i].paid;
			...
			city_pie3 () {
				var pie3 = eCharts.init(document.getElementById('citypie2')),
					...
					citypie2data = [];
				for (var i in _this.city_data) {
						var item = {name: i, value: _this.city_data[i].opened};
						_this.citypie2total += _this.city_data[i].opened;
			...
			dis_pie1 () {
				...
				for (var i in _this.disctrict_data) {
						var item = {name: i, value: _this.disctrict_data[i].white_list};
						_this.dispie1total += _this.disctrict_data[i].white_list;
			...
			dis_pie2() {
				var pie3 = eCharts.init(document.getElementById('dispie2')),
					...
					dispie2data = [];
				for (var i in _this.disctrict_data) {
						var item = {name: i, value: _this.disctrict_data[i].opened};
						_this.dispie2total += _this.disctrict_data[i].opened;
			...
			dis_pie3 () {
				var pie3 = eCharts.init(document.getElementById('dispie3')),
					...
					dispie3data = [];
				for (var i in _this.disctrict_data) {
						var item = {name: i, value: _this.disctrict_data[i].paid};
						_this.dispie3total += _this.disctrict_data[i].paid;
			...
			getpro_data () {
				...
				_this.$http({
					...
					// url: './mock/pie1.json'
					method: 'POST',
					url: '/bi/loans/user/layout',
					body: _this.pro_params,
					header: '	Accept application/json'
				}).then(response => {
					if (response.body.code == 0) {
						_this.pro_data = response.body.data;
						_this.pro_pie1();_this.pro_pie2();_this.pro_pie3();
						_this.pro_dataList = [];
						for (var i in _this.pro_data) {
							 var item = {name: i,whiteList: _this.pro_data[i].white_list, dredged: _this.pro_data[i].opened, paid: _this.pro_data[i].paid};
							 _this.pro_dataList.push(item);
						
							 _this.pro_rate1 = _this.pro_dataList[0].whiteList;
							 _this.pro_rate2 = _this.pro_dataList[0].dredged;
							 _this.pro_rate3 = _this.pro_dataList[0].paid;
						}
			...
			getcity_data (){
				...
				_this.$http({
					...
					if (response.body.code == 0) {
						_this.city_data = response.body.data;
						_this.city_pie1();_this.city_pie2();_this.city_pie3();
						_this.citydata = [];
						for (var i in _this.city_data) {
							var item = {name: i,whiteList: _this.city_data[i].white_list, dredged:_this.city_data[i].opened, paid: _this.city_data[i].paid}
							_this.citydata.push(item);
							_this.city_rate1 = _this.citydata[0].whiteList;
							_this.city_rate2 = _this.citydata[0].dredged;
							_this.city_rate3 = _this.citydata[0].paid;
						}
			...
			getdis_data () {
				...
				_this.$http({
					...
					if (response.body.code == 0) {
						_this.disctrict_data = response.body.data;
						_this.dis_pie1();_this.dis_pie2();_this.dis_pie3();
						_this.disctrictdata = [];
						for (var i in _this.disctrict_data) {
							var item = {city: i,whiteList: _this.disctrict_data[i].white_list, dredged:_this.disctrict_data[i].opened, paid: _this.disctrict_data[i].paid}
							...
							_this.dis_rate3 = _this.disctrictdata[0].paid;
						}
